...
contract LastWill is SoftDestruct, Checkable, ERC223Receiver {
  ...
  /**
   * @dev Rejects not listed tokens.
   */
  function tokenFallback(address, uint, bytes) public {
    require(internalIsTokenAddressAlreadyInList(msg.sender));
  }

  /**
   * @dev Returns added token addresses.
   */
  ...
  function internalCalculateAmounts(uint _balance) internal view returns (uint[] amounts) {
    ...
    for (uint i = 0; i < percents.length; i++) {
      ...
      uint amount = _balance * percents[i].percent / 100;
      amounts[i] = amount;
      remainder -= amount;
    }
  }
  ...
  /**
   * @dev Distribute funds between recipients in corresponding by percents.
   */
  function internalDistributeFunds() internal {
    uint[] memory amounts = internalCalculateAmounts(address(this).balance);
   ...
   */
  function internalDistributeTokens() internal {
    for (uint i = 0; i < tokenAddresses.length; i++) {
      ERC20Basic token = ERC20Basic(tokenAddresses[i]);
      uint[] memory amounts = internalCalculateAmounts(token.balanceOf(this));
  ...
  function internalAction() internal {
    ...
    internalDistributeTokens();
  }

  /**
   * @dev Is token contract address added.
   */
  function internalIsTokenAddressAlreadyInList(address _contract) internal view returns (bool) {
    for (uint i = 0; i < tokenAddresses.length; i++) {
      if (tokenAddresses[i] == _contract) {
        return true;
      }
